...
LOG_FILE="/var/log/create-lxc-web-php55-$(date '+%Y%m%d-%H%M%S').log"
...
set_defaults(){ CTID=$(pvesh get /cluster/nextid 2>/dev/null || echo 100); HOSTNAME="ubuntu-web-php55"; ROOTFS_STORAGE="local-lvm"; TEMPLATE_STORAGE="local"; CPU_CORES=2; MEMORY_MB=4096; SWAP_MB=512; DISK_GB=32; BRIDGE="vmbr0"; VLAN_TAG=0; NETWORK_MODE="dhcp"; NET_IP="dhcp"; GATEWAY=""; DNS_SERVER="8.8.8.8"; ONBOOT=1; UNPRIVILEGED=1; FIREWALL=0; ADMIN_USER="admin"; DB_ADMIN_USER="dbadmin"; }

choose_mode(){
  MODE=$(wt --title "CHOIX DU PARAMÉTRAGE" --radiolist \
    "Sélectionne le mode de configuration.\n\n↑/↓ : déplacer   Espace : cocher   Tab : OK   Entrée : valider" \
    20 96 8 \
    "1" "Paramètres par défaut - identifiants générés automatiquement" ON \
    "2" "Paramètres par défaut - saisie manuelle des identifiants" OFF \
    "3" "Paramètres avancés - configuration complète et identifiants personnalisés" OFF \
    "4" "Quitter" OFF) || exit 0
  MODE=${MODE//\"/}
  [[ -n "$MODE" ]] || { wt_msg "Sélection obligatoire" "Coche un choix avec la barre espace."; choose_mode; return; }
  case "$MODE" in
    1|2|3) ;;
    4) exit 0 ;;
...
validate_storage(){ local t; t=$(storage_type "$ROOTFS_STORAGE"); if [[ "$t" == nfs || "$t" == cifs ]]; then msg_warn "Stockage distant sélectionné : $ROOTFS_STORAGE ($t). La création sera tentée normalement par Proxmox."; fi; }
...
default_credentials(){ ADMIN_USER="admin"; DB_ADMIN_USER="dbadmin"; CT_ROOT_PASSWORD=$(random_password); ADMIN_PASSWORD=$(random_password); DB_ADMIN_PASSWORD=$(random_password); CODE_SERVER_PASSWORD=$(random_password); msg_ok "Identifiants alphanumériques de 8 caractères générés automatiquement."; }
confirm(){ local credential_mode; [[ "$MODE" == 1 ]] && credential_mode="Générés automatiquement et inscrits dans les notes Proxmox" || credential_mode="Personnalisés"; wt_yesno "CONFIRMATION - PHP HÉRITÉ" "ATTENTION : PHP 5.5.38 n'est plus maintenu depuis 2016.\nCe serveur doit rester isolé d'Internet ou protégé par un filtrage strict.\n\nVMID : $CTID\nNom : $HOSTNAME\nStockage racine : $ROOTFS_STORAGE\nTemplate : $TEMPLATE_STORAGE\nCPU : $CPU_CORES\nRAM : $MEMORY_MB Mo\nDisque : $DISK_GB Go\nBridge : $BRIDGE\nRéseau : $NETWORK_MODE\nDNS : $DNS_SERVER\nUtilisateur Linux : $ADMIN_USER\nUtilisateur MariaDB : $DB_ADMIN_USER\nIdentifiants : $credential_mode\n\nCréer le conteneur ?" || exit 0; }
...
PHP_IMAGE="local/php:5.5.38-fpm"
PHP_CONTAINER="legacy-php55-fpm"
...
step_system(){
  ...
  apt-get install -y ca-certificates curl openssl sudo unattended-upgrades
}

step_apache_php(){
  apt-get install -y acl apache2 docker.io
  mkdir -p /etc/docker /opt/php55-fpm /var/www/html
...
JSON
  ...
  cat >/opt/php55-fpm/Dockerfile <<'DOCKERFILE'
FROM php:5.5.38-fpm
RUN docker-php-ext-install -j"$(nproc)" mysqli pdo_mysql \
 && sed -ri 's!^listen = .*!listen = 127.0.0.1:9000!' /usr/local/etc/php-fpm.d/zz-docker.conf
COPY legacy.ini /usr/local/etc/php/conf.d/zz-legacy.ini
DOCKERFILE
  cat >/opt/php55-fpm/legacy.ini <<'PHPINI'
expose_php = Off
display_errors = Off
log_errors = On
...
PHPINI
  docker build --pull=false --tag "$PHP_IMAGE" /opt/php55-fpm
  docker run --rm "$PHP_IMAGE" php -r 'exit(PHP_VERSION === "5.5.38" ? 0 : 1);'
  docker run --rm "$PHP_IMAGE" php -r 'exit(extension_loaded("mysqli") && extension_loaded("pdo_mysql") ? 0 : 1);'

  cat >/etc/systemd/system/legacy-php55-fpm.service <<EOF
[Unit]
Description=PHP 5.5.38 FPM isolé dans Docker
...
Restart=always
RestartSec=5
ExecStartPre=-/usr/bin/docker rm -f $PHP_CONTAINER
ExecStart=/usr/bin/docker run --name $PHP_CONTAINER --network host --volume /var/www/html:/var/www/html $PHP_IMAGE
ExecStop=/usr/bin/docker stop --time 10 $PHP_CONTAINER
...
EOF
  ...
  cat >/etc/apache2/conf-available/php55-fpm.conf <<'APACHE'
<FilesMatch "\.php$">
...
<html lang="fr">
<meta charset="utf-8">
<title>Serveur Web PHP 5.5.38</title>
<h1>Serveur Web Ubuntu</h1>
<p>Apache et PHP <?php echo htmlspecialchars(PHP_VERSION, ENT_QUOTES, 'UTF-8'); ?> sont opérationnels.</p>
PHP
  systemctl daemon-reload
  systemctl enable --now legacy-php55-fpm.service
...
step_phpmyadmin(){
  local archive="phpMyAdmin-${PHPMYADMIN_VERSION}-all-languages.tar.gz"
  local base_url="https://files.phpmyadmin.net/phpMyAdmin/${PHPMYADMIN_VERSION}"
  local blowfish_secret
  cd /tmp
  curl -fsSLO "${base_url}/${archive}"
  curl -fsSLO "${base_url}/${archive}.sha256"
  sha256sum -c "${archive}.sha256"
  rm -rf "/var/www/html/phpmyadmin" "/tmp/phpMyAdmin-${PHPMYADMIN_VERSION}-all-languages"
  tar -xzf "$archive"
...
EOF
  ...
  chmod 0750 /var/www/html/phpmyadmin/tmp
  chmod 0640 /var/www/html/phpmyadmin/config.inc.php
}
...
step_samba(){
  ...
  setfacl -Rm u:www-data:rwX,d:u:www-data:rwX /var/www/html
...
step_final(){
  ...
  systemctl is-active --quiet legacy-php55-fpm.service
  systemctl is-active --quiet mariadb
  systemctl is-active --quiet ssh
  systemctl is-active --quiet smbd
  systemctl is-active --quiet "code-server@$ADMIN_USER.service"
  docker exec "$PHP_CONTAINER" php -r 'exit(PHP_VERSION === "5.5.38" ? 0 : 1);'
  curl -fsS http://127.0.0.1/ | grep -q 'PHP 5.5.38'
  curl -fsS http://127.0.0.1/phpmyadmin/ >/dev/null
  rm -f /root/.lxc-web-install-credentials
}

printf 'Installation du serveur Web Ubuntu avec PHP 5.5.38\n'
run_step "Configuration du réseau" step_network
run_step "Mise à jour du système" step_system
run_step "Apache et PHP 5.5.38 isolé" step_apache_php
...
<p><strong>Attention :</strong> PHP 5.5.38 est obsolète. Ne pas exposer ce serveur directement à Internet.</p>

</div>

## Services
- Apache - http://${ip}
- PHP 5.5.38 via FPM isolé
- MariaDB
...
finish(){ local ip; ip=$(container_ip); ip=${ip:-adresse_non_detectee}; write_notes "$ip" "Installation terminée"; CT_CREATION_STARTED=0; wt_msg "INSTALLATION TERMINÉE" "Conteneur prêt avec PHP 5.5.38.\n\nVMID : $CTID\nNom : $HOSTNAME\nIP : $ip\n\nApache : http://$ip\nphpMyAdmin : http://$ip/phpmyadmin\ncode-server : http://$ip:8680\nSamba : \\\\$ip\\Web\nSSH : $ADMIN_USER@$ip\n\nSécurité : ne pas exposer directement ce serveur à Internet."; msg_ok "LXC $CTID prêt : http://$ip"; }
