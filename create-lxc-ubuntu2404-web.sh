...
choose_mode(){ MODE=$(wt --title "PARAMÈTRES" --radiolist "Choisis le mode.\n\n↑/↓ : déplacer   Espace : cocher   Tab : OK   Entrée : valider" 19 88 5 "1" "Paramètres par défaut — identifiants générés automatiquement" ON "2" "Paramètres avancés — configuration et identifiants personnalisés" OFF "3" "Quitter" OFF) || exit 0; [[ -n "$MODE" ]] || { wt_msg "Sélection obligatoire" "Coche un choix avec la barre espace."; choose_mode; return; }; [[ "$MODE" != 3 ]] || exit 0; }
...
configure_credentials(){ if [[ "$MODE" == 1 ]]; then default_credentials; else advanced_credentials; fi; }
...
main(){ require_environment; touch "$LOG_FILE"; chmod 600 "$LOG_FILE"; exec > >(tee -a "$LOG_FILE") 2>&1; set_defaults; choose_mode; choose_storages; [[ "$MODE" == 2 ]] && advanced_configuration; validate_bridge; validate_storage; configure_credentials; confirm; download_template; build_installer; create_container; configure_guest_network; install_inside; finish; }
main "$@"
